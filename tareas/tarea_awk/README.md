# Programa de AWK para Graficar Funciones Matemáticas
## Integrantes:
- Luis Sánchez
- David bermudez
- Santiago Ospina

## Descripción
Este programa utiliza AWK para graficar funciones matemáticas en la terminal. Permite visualizar curvas de manera sencilla sin necesidad de software adicional.

## Instrucciones de Ejecución

1. **Descargar el archivo** `graficador.awk`.
2. **Conceder permisos de ejecución** con el siguiente comando:
   ```sh
   chmod +x graficador.awk
3. **Ejecutar el script** en la terminal con el siguiente comando:
    ```sh
    ./graficador.awk
